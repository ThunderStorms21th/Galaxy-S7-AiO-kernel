...
static void update_cpu_max_freq(unsigned int cpu)
{
	...
	if (debug_mask) {
		pr_info("%s: Setting Max Freq for CPU%u: %u Hz\n",
...
static void update_cpu_min_freq(unsigned int cpu)
{
	...
	mutex_lock(&per_cpu(limit, cpu).msm_limiter_mutex);
	if (debug_mask) {
		pr_info("%s: Setting Min Freq for CPU%u: %u Hz\n",
...
static void msml_early_suspend(struct power_suspend *handler)
{
	unsigned int cpu;
...
static void msml_late_resume(struct power_suspend *handler)
{
	unsigned int cpu;
	const bool hotplug_state = is_hotplug_state();
	suspend_status = 0;
...
static struct power_suspend msml_power_suspend_handler = {
	.suspend = msml_early_suspend,
	.resume = msml_late_resume,
};
...
static int msm_limiter_init(void)
{
	...
	if (freq_control)
		msm_limiter_start();
		register_power_suspend(&msml_power_suspend_handler);
	return ret;
err_dev:
...
static void msm_limiter_exit(void)
{
	...
	if (!freq_control)
		msm_limiter_stop();
		unregister_power_suspend(&msml_power_suspend_handler);
