/*
 * drivers/cpufreq/cpufreq_thunderstorms.c
 *
...
struct cpufreq_thunderstorms_cpuinfo {
	...
	struct cpufreq_thunderstorms_tunables *cached_tunables;
};
...
static DEFINE_PER_CPU(struct cpufreq_thunderstorms_cpuinfo, cpuinfo);
...
#define DEFAULT_SLEEP_MIN_FREQ                  130000
 ...
 */
#define DEFAULT_SLEEP_PREV_FREQ			130000
...
struct cpufreq_thunderstorms_tunables {
	int usage_count;
...
static struct cpufreq_thunderstorms_tunables *common_tunables;

static struct attribute_group *get_sysfs_attr(void);

/* Round to starting jiffy of next evaluation window */
static u64 round_to_nw_start(u64 jif,
			     struct cpufreq_thunderstorms_tunables *tunables)
{
...
static void cpufreq_thunderstorms_timer_resched(
	struct cpufreq_thunderstorms_cpuinfo *pcpu)
{
	struct cpufreq_thunderstorms_tunables *tunables =
 ...
 * function.
 */
static void cpufreq_thunderstorms_timer_start(
	struct cpufreq_thunderstorms_tunables *tunables, int cpu)
{
	struct cpufreq_thunderstorms_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int freq_to_targetload(
	struct cpufreq_thunderstorms_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
 ...
 */
static unsigned int choose_freq(struct cpufreq_thunderstorms_cpuinfo *pcpu,
...
static unsigned int fastlane_freq(struct cpufreq_thunderstorms_cpuinfo *pcpu,
...
static void cpufreq_thunderstorms_timer(unsigned int freq_table,
			unsigned long data)
{
	u64 now;
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer)) {
		pcpu->last_evaluated_jiffy = get_jiffies_64();
		cpufreq_thunderstorms_timer_resched(pcpu);
	}
...
static void cpufreq_thunderstorms_idle_end(void)
{
	struct cpufreq_thunderstorms_cpuinfo *pcpu =
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_thunderstorms_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static void cpufreq_thunderstorms_get_policy_info(struct cpufreq_policy *policy,
...
static void cpufreq_thunderstorms_adjust_cpu(unsigned int cpu,
					   struct cpufreq_policy *policy)
{
	struct cpufreq_thunderstorms_cpuinfo *pcpu;
	u64 hvt, fvt;
	...
	cpufreq_thunderstorms_get_policy_info(policy, &max_freq, &hvt, &fvt);
...
static int cpufreq_thunderstorms_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			if (likely(down_read_trylock(&pcpu->enable_sem))) {
				if (likely(pcpu->governor_enabled))
					cpufreq_thunderstorms_adjust_cpu(cpu,
							pcpu->policy);
				up_read(&pcpu->enable_sem);
			}
...
static int cpufreq_thunderstorms_notifier(
	struct notifier_block *nb, unsigned long val, void *data)
{
	...
	if (val == CPUFREQ_PRECHANGE) {
		...
		for_each_cpu(cpu, pcpu->policy->cpus) {
			struct cpufreq_thunderstorms_cpuinfo *pjcpu =
				&per_cpu(cpuinfo, cpu);
			if (cpu != freq->cpu) {
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_thunderstorms_notifier,
};
...
static ssize_t show_above_hispeed_delay(
	struct cpufreq_thunderstorms_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
	unsigned long flags;
...
static ssize_t show_hispeed_freq(struct cpufreq_thunderstorms_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->hispeed_freq);
}

static ssize_t store_hispeed_freq(struct cpufreq_thunderstorms_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_go_hispeed_load(struct cpufreq_thunderstorms_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->go_hispeed_load);
}

static ssize_t store_go_hispeed_load(struct cpufreq_thunderstorms_tunables
...
static ssize_t show_min_sample_time(struct cpufreq_thunderstorms_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->min_sample_time);
}

static ssize_t store_min_sample_time(struct cpufreq_thunderstorms_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_thunderstorms_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
}

static ssize_t store_timer_rate(struct cpufreq_thunderstorms_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_timer_slack(struct cpufreq_thunderstorms_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->timer_slack_val);
}

static ssize_t store_timer_slack(struct cpufreq_thunderstorms_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_io_is_busy(struct cpufreq_thunderstorms_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_thunderstorms_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_align_windows(struct cpufreq_thunderstorms_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->align_windows);
}

static ssize_t store_align_windows(struct cpufreq_thunderstorms_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate_screenoff(struct cpufreq_thunderstorms_tunables
               *tunables, char *buf)
{
       return sprintf(buf, "%lu\n", tunables->timer_rate_screenoff);
}

static ssize_t store_timer_rate_screenoff(struct cpufreq_thunderstorms_tunables
               *tunables, const char *buf, size_t count)
{
       int ret;
       unsigned long val, val_round;
...
static struct attribute *thunderstorms_attributes_gov_sys[] = {
...
static struct attribute_group thunderstorms_attr_group_gov_sys = {
	.attrs = thunderstorms_attributes_gov_sys,
	.name = "thunderstorms",
};

/* Per policy governor instance */
static struct attribute *thunderstorms_attributes_gov_pol[] = {
...
static struct attribute_group thunderstorms_attr_group_gov_pol = {
	.attrs = thunderstorms_attributes_gov_pol,
	.name = "thunderstorms",
};
...
static struct attribute_group *get_sysfs_attr(void)
{
	if (have_governor_per_policy())
		return &thunderstorms_attr_group_gov_pol;
	else
		return &thunderstorms_attr_group_gov_sys;
}

static int cpufreq_thunderstorms_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	if (val == IDLE_END)
		cpufreq_thunderstorms_idle_end();

	return 0;
}

static struct notifier_block cpufreq_thunderstorms_idle_nb = {
	.notifier_call = cpufreq_thunderstorms_idle_notifier,
};
...
static struct cpufreq_thunderstorms_tunables *alloc_tunable(
					struct cpufreq_policy *policy)
{
	struct cpufreq_thunderstorms_tunables *tunables;
...
static struct cpufreq_thunderstorms_tunables *restore_tunables(
						struct cpufreq_policy *policy)
{
	int cpu;
...
static int cpufreq_governor_thunderstorms(struct cpufreq_policy *policy, unsigned int freq_table,
		unsigned int event)
{
	int rc;
	unsigned int j;
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (!policy->governor->initialized) {
			idle_notifier_register(&cpufreq_thunderstorms_idle_nb);
		...
		if (!--tunables->usage_count) {
			if (policy->governor->initialized == 1) {
				cpufreq_unregister_notifier(&cpufreq_notifier_block,
						CPUFREQ_TRANSITION_NOTIFIER);
				idle_notifier_unregister(&cpufreq_thunderstorms_idle_nb);
			}
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			pcpu->last_evaluated_jiffy = get_jiffies_64();
			cpufreq_thunderstorms_timer_start(tunables, j);
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_THUNDERSTORMS
static
#endif
struct cpufreq_governor cpufreq_gov_thunderstorms = {
	.name = "thunderstorms",
	.governor = cpufreq_governor_thunderstorms,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_thunderstorms_nop_timer(unsigned long data)
{
}

static int __init cpufreq_thunderstorms_init(void)
{
	unsigned int i;
	struct cpufreq_thunderstorms_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_thunderstorms_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_thunderstorms_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_thunderstorms_speedchange_task, NULL,
			       "cfthunderstorms");
	...
	return cpufreq_register_governor(&cpufreq_gov_thunderstorms);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_THUNDERSTORMS
fs_initcall(cpufreq_thunderstorms_init);
#else
module_init(cpufreq_thunderstorms_init);
#endif

static void __exit cpufreq_thunderstorms_exit(void)
{
	int cpu;
	struct cpufreq_thunderstorms_cpuinfo *pcpu;

	cpufreq_unregister_governor(&cpufreq_gov_thunderstorms);
...
module_exit(cpufreq_thunderstorms_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_AUTHOR("mydongistiny <jaysonedson@gmail.com>");
MODULE_AUTHOR("@nala <nala-s@wp.pl>");
MODULE_DESCRIPTION("'cpufreq_thunderstorms' - A cpufreq governor for "
