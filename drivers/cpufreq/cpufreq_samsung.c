...
/*
static inline cputime64_t get_cpu_idle_time1_jiffy(unsigned int cpu,
							cputime64_t *wall)
{
...
{
	u64 idle_time = get_cpu_idle_time1_us(cpu, wall);

	if (idle_time == -1ULL)
		return get_cpu_idle_time1_jiffy(cpu, wall);
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		...
		if (dbs_tuners_ins.ignore_nice)
			...
			dbs_info->prev_cpu_nice = kstat_cpu(j).nice;
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_idle_time = get_cpu_idle_time1(j, &cur_wall_time);
...
static void dbs_refresh_callback(struct work_struct *unused)
{
	...
	if (policy->cur < policy->max) {
		...
		__cpufreq_driver_target(policy, policy->max,
					CPUFREQ_RELATION_L);
		this_dbs_info->prev_cpu_idle = get_cpu_idle_time1(cpu,
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_dbs_info->prev_cpu_idle = get_cpu_idle_time1(j,
						&j_dbs_info->prev_cpu_wall);
			if (dbs_tuners_ins.ignore_nice) {
				j_dbs_info->prev_cpu_nice =
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	idle_time = get_cpu_idle_time1_us(cpu, &wall);
	put_cpu();
	if (idle_time != -1ULL) {
